...
# 2025/12/25
1. 原数据集有以下的问题：由于先前Node状态的合并，导致许多应该出现新内容的操作最后被判断没有出现新的内容，比如说SWIPE，以及触发按钮之之后导致的状态变化。
2. 有很多语义相同的、但实际上又不太相同的任务，比如在许多页面都能触发搜索栏这个按钮；在缺少具体语境（比如开始页面位于哪个页面功能簇？）难以选择具体是哪一个。或者进一步提升抽象程度，生成和具体页面内容无关的抽象，这和我们现在的想法符合吗？
为什么KG-RAG不会出现这种情况？因为KG-RAG是固定起点（主页面/初始页面）的，所以它的子任务都是从初始页面出发并总结，并不会出现以下语义混淆的情况。
3. prompt是否可以做好一点？为了避免语义混淆，是否在总结功能簇的summary、以及功能簇中的子任务等，要强调差异化？或者生成完后再用大模型检查一遍？
4. 核心改进之一：现在的分簇效果很糟糕，首先簇的大小差别太大，有的特别大，有的特别小；其次不符合语义，有的页面相似却分到了不同簇，有的则是反过来


第一阶段：夯实底层（基础数据质量）
解决问题 1（Node 合并过度）
似乎做不了什么。因为原来已经是合并的状态，因此没办法改
可以过滤一些 无效/重复 操作

第二阶段：优化结构（分簇逻辑改进）
还可以添加总结出语义之后剔除的？有必要吗？


第三阶段：语义增强（Prompt 与 上下文）
...
我记得应该是总结完了每个簇的功能。
我记得应该是上周跟老师说过，要新建立起一个给当前状态（截图/XML）后，能在原始UTG中大概定位的机制。
今天就完成这个吧！
